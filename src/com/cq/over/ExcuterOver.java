...
public class ExcuterOver extends SimpleExcuter {

	public ExcuterOver(int h, int m, int s) {
		super(h, m, s);
	}

	public static void main(String[] args) {
		new ExcuterOver(1,6,0);
	}
	...
	public void doCheck() {
		...
		try {	
			...
			wdwE.until(ExpectedConditions.presenceOfElementLocated(By.id("overtime_checkout_btn")));
			exc.findElementById("overtime_checkout_btn").click();
			
			int aTimes = 0;
			do{
				Thread.sleep(3000);
				try {
					Alert alert = exc.switchTo().alert();
					alert.accept();
				} catch(Exception e) {
					log.error(e);
				}
				...
				Thread.sleep(3000);
				exc.findElement(By.name("overtime_check_out_save")).click();
				Thread.sleep(30000);
